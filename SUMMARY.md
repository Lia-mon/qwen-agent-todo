...
- **Re-emergence** — when a cycle cross arrives while the task is completed, `runScheduledReemergence()` moves it back to active (`completed = 0`, `completedAt = null`) and re-anchors `nextRepeatDate` to the next cross from now — re-emerged tasks carry a next cycle just like any other active repeatable. The stored cross is consumed by the re-emergence itself; any crosses after it (while the app was closed) are added to `repeatStack`, so a task re-emerging after a long gap arrives stacked, exactly as if the app had been open and it had re-emerged on the first cross and stacked the rest. It scans the `nextRepeatDate` index for records ≤ now (`dbGetDueTodos`) **across all profiles**, re-renders, and logs counts. Runs on `init()` and on every `activateProfile()` (profile switch / full-data restore).
...
15. **Re-emergence consumes one cross** — a completed task re-emerges on its stored cross (that cross is not stacked); crosses after it, while the app was closed, stack on re-emergence, matching what an always-open app would have accumulated. Time spent in trash still doesn't stack (restoring re-anchors from now, keeping any existing stack).
